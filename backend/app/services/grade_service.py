...
{{
  ...
  "suggestions": "教学改进建议，3-4条，每条80字以内。必须结合上方高分学生和需关注学生的实际情况，点名分析具体学生的薄弱环节（如：某某某期末分偏低需加强、某某某平时分高但期中滑坡需关注波动），给出针对性辅导策略。用\\n\\n分隔各条建议。"
}}
...
class GradeService:
    ...
    async def generate_ai_report(
        ...
        # 从实际数据中提取高分和需关注学生（确定性计算，不交给AI）
        sorted_items = sorted(items, key=lambda x: x.get("total_score", 0), reverse=True)
        top_students_str = "\n".join([
            f"  {i+1}. {item.get('student_name', '未知')}（总评: {item.get('total_score', '-')}分）"
        ...
        prompt = REPORT_PROMPT.format(
            ...
            bottom_students=bottom_students_str
        )
        ...
        try:
            ...
            # 归一化 suggestions：AI 可能返回数组或字符串，统一转为字符串
            suggestions = report_data.get("suggestions", "")
            if isinstance(suggestions, list):
                suggestions = "\n\n".join(suggestions)

            self.logger.info(f"[Grade] AI report parsed successfully, high={high_performers}, attention={needs_attention}")
